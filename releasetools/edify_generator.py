...
class EdifyGenerator(object):
  ...
  def AssertSomeFingerprint(self, *fp):
    ...
    cmd = (' ||\n    '.join([('file_getprop("/system/build.prop", "ro.build.fingerprint") == "%s"') % i
                             for i in fp]) +
  ...
  def AssertSomeThumbprint(self, *fp):
    ...
    cmd = (' ||\n    '.join([('file_getprop("/system/build.prop", "ro.build.thumbprint") == "%s"') % i
                             for i in fp]) +
  ...
  def UnpackPackageDir(self, src, dst):
    ...
    self.script.append(
            'package_extract_dir("%s", "%s") || abort("Failed to extract dir from \\"%s\\" to \\"%s\\".");' %
            (src, dst, src, dst))
    # END
  ...
  def ApplyPatch(self, srcfile, tgtfile, tgtsize, tgtsha1, *patchpairs):
    ...
    for i in range(0, len(patchpairs), 2):
      cmd.append(',\0%s, package_extract_file("%s")' % patchpairs[i:i+2])
      patchfiles = patchfiles.join('%s ' % patchpairs[i+1])
    cmd.append(') || abort("Failed to apply patch \\"%s\\".");' % (patchfiles,))
    # END
    cmd = "".join(cmd)
    self.script.append(self.WordWrap(cmd))
